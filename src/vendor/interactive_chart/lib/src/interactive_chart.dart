  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final candlesPercentage = candlesInRange.length / (widget.candles?.length ?? 1.0);
        ...
        double? highest(CandleData c) {
          if (c.amheragAbove != null) return c.close! + (c.amheragAbove!.length * candlesPercentage + 2) * widget.amheragHeight!;
          // if (c.amheragAbove != null) return c.close! + 8 * widget.amheragHeight!;
        ...
        double? lowest(CandleData c) {
          if (c.amheragBelow != null) return c.close! - (c.amheragBelow!.length * candlesPercentage + 2) * widget.amheragHeight!;
          // if (c.amheragBelow != null) return c.close! - 8 * widget.amheragHeight!;
        ...
        final child = TweenAnimationBuilder(
          tween: PainterParamsTween(
              ...
              maxVol: maxVol,
              minVol: minVol,
              amheragHeight: widget.amheragHeight,
              tp: widget.tp,
              sl: widget.sl,
              newTp: widget.newTp,
              newSl: widget.newSl,
